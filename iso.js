var directions = {
west:
    { offset: 0, x: -2, y: 0, opposite: 'east' },
northWest:
    { offset: 32, x: -2, y: -1, opposite: 'southEast' },
north:
    { offset: 64, x: 0, y: -2, opposite: 'south' },
northEast:
    { offset: 96, x: 2, y: -1, opposite: 'southWest' },
east:
    { offset: 128, x: 2, y: 0, opposite: 'west' },
southEast:
    { offset: 160, x: 2, y: 1, opposite: 'northWest' },
south:
    { offset: 192, x: 0, y: 2, opposite: 'north' },
southWest:
    { offset: 224, x: -2, y: 1, opposite: 'northEast' }
};

var anims = {
idle:
    {startFrame: 0, endFrame: 4,  speed: 0.2  },
walk:
    {startFrame: 4, endFrame: 12, speed: 0.15 },
attack:
    {startFrame: 12,endFrame: 20, speed: 0.11 },
die:
    {startFrame: 20,endFrame: 28, speed: 0.2  },
shoot:
    {startFrame: 28,endFrame: 32, speed: 0.1  }
};
...
class Skeleton extends Phaser.GameObjects.Image {
    ...
    update () {
        if (this.motion === 'walk') {
            ...
            //verifica se o agente esta dentro do mapa, com pequeno ajuste no y

            pos.x = this.x;
            pos.y = this.y+25;

            if(!isInside(polygon, polygon.length, pos)) {

                // o agente fica sendo puxado para direção oposta
                this.x -= this.direction.x * this.speed;
                if (this.direction.y !== 0) {
                    this.y -= this.direction.y * this.speed;
                    this.depth = this.y + 64;
...
class Example extends Phaser.Scene {
    ...
    create () {
        ...
        // poligono que representa o mapa
        polygon = [new Point(0, 400),
                       new Point(800, 0),
                       new Point(1600, 400),
                       new Point(800,800,),
                       new Point(0, 400)];

        // ponto onde se encontra o agente.
        pos = new Point(0,0);
